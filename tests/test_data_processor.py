...
import unittest
import json
from data_processor.data_processor import callback, TEMP_THRESHOLD, HUMIDITY_THRESHOLD
...
class TestDataProcessor(unittest.TestCase):
    def test_threshold_check_temperature(self):
        ...
        ch.basic_publish.assert_called_once()  # Проверяем, что уведомление отправлено
        ...
        self.assertEqual(notification['message'], "Температура вне диапазона: 30.0°C")
    ...
    def test_threshold_check_humidity(self):
        ...
        self.assertEqual(call_args[1]['routing_key'], 'notifications')
        notification = json.loads(call_args[1]['body'])
        self.assertEqual(notification['message'], "Влажность вне диапазона: 20.0%")
        ch.basic_ack.assert_called_with(delivery_tag=method.delivery_tag)
